...
class SalesOrdersResource extends Resource
{
                ...
                Forms\Components\Fieldset::make('')
                    ->schema([
                        Forms\Components\TextInput::make('sales_order_number')
                            ->default(fn (): string => 'SO-2304'.str_pad(SalesOrder::where('team_id', Filament::getTenant()->id)->count() + 1, 3, 0, STR_PAD_LEFT)),
                ...
                TableRepeater::make('items')
                    ...
                    ->schema([
                        ...
                        Forms\Components\TextInput::make('quantity')
                            ->live(onBlur: true)
                            ->afterStateUpdated(function ($get, $set) {
                                $available = Item::find($get('item'));
                                $total = floatval($get('quantity')) * floatval($get('rate'));
                                $name = $available['part_number'] ?? $available['name'];
                                if (intval($get('quantity')) > intval($available['stock_on_hand'])) {

                                    Notification::make()
                                        ->title('Stock Limit Exceeded')
                                        ->body('Set quantity for item '.$name.' exceeds available stock')
                                        ->danger()
                                        ->actions([
                                            NotificationAction::make('create_inventory_adjustment')
                                                ->color('green')
                                                ->url(route('filament.dashboard.resources.inventory-adjustments.create', ['tenant' => Filament::getTenant()->id])),
                                        ])
                                        ->send();

                                    $set('quantity', intval($available['stock_on_hand']));
                                }
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                Tables\Columns\TextColumn::make('sales_order_date')
                    ->date()
                    ->sortable(),
                Tables\Columns\TextColumn::make('sales_order_number')
                    ->label('Sales Order#')
                    ->searchable(),
    ...
    public static function getPages(): array
    {
        return [
            ...
            'create' => Pages\CreateSalesOrders::route('/create'),
            'view' => Pages\ViewSalesOrder::route('/{record}'),
            'edit' => Pages\EditSalesOrders::route('/{record}/edit'),
        ];
    }
}
