        ...
        foreach ($packages as $package) {
            foreach ($package->items as $index => $item) {
                ...
                if ($orderedQuantity > $deliveredQuantity) {
                    array_push($backorderedItems, array("item" => $item["item"], "purchase_order_id" => $this->id, "purchase_receives_id" => $package->id, "backorder_quantity" => $orderedQuantity - $deliveredQuantity));
                }
            }
        }
