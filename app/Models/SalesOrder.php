...
class SalesOrder extends Model
{
    ...
    public function calculateBackorderedQuantities()
    {
        ...
        $packages = Packages::where('sales_order_number', $this->id)->get();
        foreach ($packages as $package) {
            foreach ($package->items as $index => $item) {
                ...
                if ($orderedQuantity > $deliveredQuantity) {
                    array_push($backorderedItems, array("item" => $item["item"], "sales_order_id" => $this->id, "package_id" => $package->id, "backorder_quantity" => $orderedQuantity - $deliveredQuantity));
                }
            }
        }
